...
func setupTest(t *testing.T, fn func(*Config)) (rootClient, nobodyClient api.LogClient, cfg *Config, teardown func()) {
	...
	var telemetryExperter *exporter.LogExporter
	if *debug {
		...
		telemetryExperter, err := exporter.NewLogExporter(exporter.Options{
			MetricsLogFile:    metricsLogFile.Name(),
			TracesLogFile:     tracesLogFile.Name(),
			ReportingInterval: time.Second,
		})
		require.NoError(t, err)
		err = telemetryExperter.Start()
		require.NoError(t, err)
	}
	...
	return rootClient, nobodyClient, cfg, func() {
		...
		if telemetryExperter != nil {
			time.Sleep(1500 * time.Millisecond)
			telemetryExperter.Stop()
			telemetryExperter.Close()
		}
	}
}
